...
async def get_smart(entity_id: str | int, smart_id: str | int) -> dict | None:
    """Get smart by given entity type id and smart item id"""
    response = await get("crm.item.get", {"entityTypeId": entity_id, "id": smart_id})
...
async def get_smarts(entity_id: str | int, filters: dict, select: list, order: Literal["ASC", "DESC"] = "ASC", limit: int | None = None) -> list[dict]:
    """
    ...
    last_smart_id = None
    while True:
        response : list[dict] | None = await post("crm.item.list", {"entityTypeId": entity_id, "filter": filters, "select": select, "order": {"ID": order}, "start": len(result)})
        if not response:
            break
